  ...
  return (
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/batches/new" className="flex items-center gap-2 bg-amber-500 hover:bg-amber-400 text-white font-semibold px-4 py-2 rounded-lg text-sm transition-colors">
          <Plus size={16} /> New Batch
        </Link>
      </div>

      <div className="flex gap-2 flex-wrap">
        <Link href="/batches" className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${!schoolFilter ? 'bg-amber-50 text-amber-700 border border-amber-200' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
          All Schools
        </Link>
        {schools?.map((s) => (
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        {batches?.length === 0 ? (
          <div className="p-10 text-center text-gray-400">
            ...
            <Link href="/batches/new" className="text-amber-600 hover:underline mt-2 inline-block">Create first weekly batch →</Link>
          </div>
        ) : (
