...
export default async function DashboardPage() {
  ...
  if (overdueBatches.length > 0) {
    ...
    alerts.push({ level: 'high', message: `${overdueBatches.length} overdue batch${overdueBatches.length > 1 ? 'es' : ''} not yet closed: ${names}`, href: '/admin/reconciliation' })
  }
  ...
  if (highVarianceSchools.length > 0) {
    ...
    alerts.push({ level: 'high', message: `High variance (>500K LBP) at: ${names}`, href: '/admin/reconciliation' })
  }

  if (openBatches.length > 0 && overdueBatches.length === 0) {
    alerts.push({ level: 'medium', message: `${openBatches.length} batch${openBatches.length > 1 ? 'es' : ''} pending reconciliation`, href: '/admin/reconciliation' })
  }
      ...
      <div className="relative rounded-2xl overflow-hidden" style={{ height: 200, background: 'linear-gradient(110deg, #78350f 0%, #92400e 30%, #b45309 60%, #d97706 85%, #f59e0b 100%)' }}>
        ...
        <div className="relative z-10 p-8 h-full flex flex-col justify-between">
          ...
          <div className="flex gap-3">
            <Link href="/admin/batches/new" className="inline-flex items-center gap-2 bg-white text-amber-800 font-semibold text-sm px-4 py-2 rounded-lg hover:bg-amber-50 transition-colors" style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.18)' }}>
              <Truck size={15} /> New Batch
            </Link>
            <Link href="/admin/reconciliation" className="inline-flex items-center gap-2 font-semibold text-sm px-4 py-2 rounded-lg transition-colors text-white" style={{ background: 'rgba(255,255,255,0.18)', border: '1px solid rgba(255,255,255,0.35)', backdropFilter: 'blur(6px)' }}>
      ...
      <div className="bg-white rounded-2xl border border-gray-200" style={{ boxShadow: '0 2px 16px rgba(0,0,0,0.06)' }}>
        <div className="p-5 border-b border-gray-100 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link href="/admin/reports" className="text-sm text-indigo-600 hover:text-indigo-700 font-medium inline-flex items-center gap-1">
              <BarChart2 size={13} /> P&amp;L Report
            </Link>
            <Link href="/admin/batches" className="text-sm text-amber-600 hover:text-amber-700 font-medium">View all batches →</Link>
          </div>
        </div>
        {schoolSummaries.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <Link href="/admin/batches/new" className="inline-flex items-center gap-2 mt-5 bg-amber-500 text-white font-semibold text-sm px-5 py-2.5 rounded-lg hover:bg-amber-400 transition-colors">
                  ...
                  return (
                    <tr key={s.batch_id} className="border-b border-gray-50 hover:bg-amber-50/40 transition-colors">
                      ...
                      <td className="px-5 py-3.5">
                        <Link href={`/admin/batches/${s.batch_id}`} className="text-xs text-amber-600 hover:underline font-medium">Details</Link>
                      </td>
                    </tr>
                  )
                })}
